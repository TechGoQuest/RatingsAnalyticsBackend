...
router.post("/org-final-list-without-language-new-code", async (req, res) => {
    ...
    if (req.body.language) {
        ...
        //Change By Mehul old => { $or: language_query_arr }
        var language_query = { $and: language_query_arr }
    ...
    if (req.body.region) {
        ...
        //Change By Mehul old => { $or: region_query_arr }
        var region_query = { $and: region_query_arr }
    ...
    if (req.body.limit || req.body.offset) {
        ...
        var data = await Merge.find(find_query, { __v: 0, _id: 0 }).skip(offset).limit(limit).sort(sort_obj);
        // console.log(data);
...
router.post("/above_6", async (req, res) => {
    var tConstArr = []
    var limit = Number(req.body.limit) // limit
    var offset = Number(req.body.offset) // offset
    if (req.body.limit || req.body.offset) {
        var count = await Above_6.countDocuments({})
        var data = await Above_6.find({}, { __v: 0, _id: 0 }).skip(offset).limit(limit);
        var lang_arr = await Language.find({}, { __v: 0, _id: 0 })
        var coun_arr = await Country.find({}, { __v: 0, _id: 0 })
        // console.log(data);
        data.forEach(element => {
...
router.post("/below_6", async (req, res) => {
    var tConstArr = []
    var limit = Number(req.body.limit) // limit
    var offset = Number(req.body.offset) // offset
    if (req.body.limit || req.body.offset) {
        var count = await Below_6.countDocuments({})
        var data = await Below_6.find({}, { __v: 0, _id: 0 }).skip(offset).limit(limit);
        // console.log(data);
        data.forEach(element => {
            tConstArr.push({ tconst: element.tconst, language: element.language, region: element.region })
        });
...
router.post("/margin_of_1", async (req, res) => {
    var tConstArr = []
    var limit = Number(req.body.limit) // limit
    var offset = Number(req.body.offset) // offset
    if (req.body.limit || req.body.offset) {
        var count = await Margin_of_1.countDocuments({})
        var data = await Margin_of_1.find({}, { __v: 0, _id: 0 }).skip(offset).limit(limit);
        // console.log(data);
        data.forEach(element => {
            tConstArr.push({ tconst: element.tconst, language: element.language, region: element.region })
        });
...
router.post("/above_500_votes", async (req, res) => {
    var tConstArr = []
    var limit = Number(req.body.limit) // limit
    var offset = Number(req.body.offset) // offset
    if (req.body.limit || req.body.offset) {
        var count = await Above_500_votes.countDocuments({})
        var data = await Above_500_votes.find({}, { __v: 0, _id: 0 }).skip(offset).limit(limit);
        // console.log(data);
        data.forEach(element => {
            tConstArr.push({ tconst: element.tconst, language: element.language, region: element.region })
        });
