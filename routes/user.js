...
router.post("/dashboard", async (req, res) => {
    ...
    if (!operation_date) {
        ...
        Cron_table.findOne({ _id: setting.cron_id }, async (err, cron) => {
            console.log("HERE", cron.is_merging_end_datetime);
            // var query = [
            //     { $match: { $and: [{ averageRating: { $lte: "7" } }, { [rating_past]: { $exists: false } }, { [rating_present]: { $exists: true } }] } }
            // ]
            // var above_6 = await Merge.aggregate(query)
            ...
            if (errors) {
                ...
                if (files.profile_pic) {
                    if (files.profile_pic.type != "image/png" && files.profile_pic.type != "image/jpeg" && files.profile_pic.type != "image/jpg") {
                        ...
                        //var newpath = "/home/projectheena/Hara/medihelp/public/images" //defines the path in the system
